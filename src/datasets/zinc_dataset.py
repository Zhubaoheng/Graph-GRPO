...
class ZINCDataset(InMemoryDataset):
    ...
    def __init__(
        ...
        if split == "train":
            ...
            self.file_idx = 2
        # PyTorch 2.6+ defaults `weights_only=True`, but these files contain full
        # torch_geometric `Data` objects, so we must load with weights_only=False.
        try:
    ...
    def download(self):
        processed_paths = [osp.join(self.processed_dir, name) for name in self.processed_file_names]
        if all(osp.exists(path) for path in processed_paths):
            return
    ...
    def process(self):
        ...
        for split in ['train', 'val', 'test']:
            ...
            logger.info('Processing %s dataset with %s molecules', split, len(indices))
...
class ZINCDataModule(MolecularDataModule):
    def __init__(self, cfg):
        ...
        logger.info("Dataset sizes: train %s, val %s, test %s", train_len, val_len, test_len)
...
def get_loader_smiles(
    ...
    if os.path.exists(smiles_path):
        ...
        logger.info("Computing dataset %s smiles...", split_key)
...
def compute_zinc_smiles(atom_decoder, train_dataloader, remove_h):
    ...
    :return:
    """
    logger.info("\tConverting ZINC dataset to SMILES for remove_h=%s...", remove_h)
    ...
    for i, data in enumerate(train_dataloader):
        ...
        for l, molecule in enumerate(molecule_list):
            ...
            if smile is not None:
                ...
                if len(mol_frags) > 1:
                    logger.info("Disconnected molecule: %s %s", mol, mol_frags)
                    disconnected += 1
            else:
                logger.warning("Invalid molecule obtained.")
                invalid += 1
    ...
    logger.warning("Number of invalid molecules: %s", invalid)
    logger.info("Number of disconnected molecules: %s", disconnected)
    return mols_smiles
