...
class TLSGraphDataset(InMemoryDataset):
    ...
    def download(self):
        ...
        for i, key in enumerate(["train", "val", "test"]):
            ...
            logger.info("Created %s dataset (len: %s)", key, len(all_data))
...
class CellGraph(nx.Graph):
    ...
    def plot_graph(
        ...
        if save_path is not None:
            ...
            if verbose:
                logger.info("Saved graph plot to %s", save_path)
            plt.close()
        else:
            plt.show()
    ...
    def compute_tls_features(
        ...
        if denominator < min_num_gamma_edges:
            ...
            if verbose:
                logger.warning("WARNING: too few gamma edges. TLO feature set to -1.")
        else:
            tlo_dict = {}
            if denominator == 0:
