...
class GuacamolDataset(InMemoryDataset):
    ...
    def __init__(
        ...
        # NOTE:
        # PyTorch 2.6+ changed the default of `weights_only` in `torch.load`
        # from False to True. Our processed dataset files contain full
        # torch_geometric `Data` objects, not just tensor weights, so we must
        # explicitly disable `weights_only` to load them.
        #
        # This is safe here because the files are generated locally by this
        # code (see `process()` below) and thus are trusted.
        try:
    ...
    def download(self):
        ...
        logger.info("Dataset download successful. Hashes are correct.")
    ...
    def process(self):
        ...
        if self.filter_dataset:
            ...
            logger.info(smiles_save_path)
            with open(smiles_save_path, "w") as f:
                f.writelines("%s\n" % s for s in smiles_kept)
            logger.info("Number of molecules kept: %s / %s", len(smiles_kept), len(smile_list))



class MockGuacamolDataset(InMemoryDataset):
    """
    Mock dataset for Guacamol MPO tasks where we don't need training data.
    Provides minimal interface to satisfy type checks.
    """
    ...
    def get(self, idx):
        # Return a dummy graph with correct dimensions for Guacamol
        # 12 atom types, 5 edge types (4 bonds + None)
        ...
        # Bond type 1 (Single) -> One-hot index 1 (considering 0 is None?) 
        # Actually Guacamol dataset process uses:
        # edge_type += 2 * [bonds[bond.GetBondType()] + 1]
        # And edge_attr = F.one_hot(edge_type, num_classes=len(bonds) + 1)
        # bonds types are 0,1,2,3. So indices are 1,2,3,4. Index 0 is likely "no bond" in dense rep, 
        # but in sparse edge_attr it is just the feature.
        # Let's verify standard Guacamol processing in this file.
        # Line 203: edge_attr = F.one_hot(edge_type, num_classes=len(bonds) + 1)
        # len(bonds) is 4. num_classes is 5.
        # So we provided a 5-dim edge attr.
        edge_attr = torch.zeros((2, 5), dtype=torch.float)
        edge_attr[:, 1] = 1.0 # Single bond (index 1)
...
class GuacamolDataModule(MolecularDataModule):
    def __init__(self, cfg):
        ...
        # Check if we should use empty dataset (for MPO/Generation tasks)
        use_empty = cfg.dataset.get("empty", False) or cfg.dataset.get("name") == "guacamol_mpo"
        
        if use_empty:
             logger.warning("WARNING: Using MockGuacamolDataset (Empty) for MPO task.")
             datasets = {
                "train": MockGuacamolDataset("train", root=root_path),
                "val": MockGuacamolDataset("val", root=root_path),
                "test": MockGuacamolDataset("test", root=root_path),
            }
        else:
            datasets = {
