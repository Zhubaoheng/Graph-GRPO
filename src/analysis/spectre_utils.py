...
def degree_stats(graph_ref_list, graph_pred_list, is_parallel=True, compute_emd=False):
    ...
    if PRINT_TIME:
        logger.debug("Time computing degree mmd: %s", elapsed)
    return mmd_dist
...
def eigval_stats(
    ...
    elapsed = datetime.now() - prev
    if PRINT_TIME:
        logger.debug("Time computing eig mmd: %s", elapsed)
    return mmd_dist
...
def spectral_filter_stats(
    ...
    elapsed = datetime.now() - prev
    if PRINT_TIME:
        logger.debug("Time computing spectral filter stats: %s", elapsed)
    return mmd_dist
...
def spectral_stats(
    ...
    elapsed = datetime.now() - prev
    if PRINT_TIME:
        logger.debug("Time computing spectral mmd: %s", elapsed)
    return mmd_dist
...
def clustering_stats(
    ...
    elapsed = datetime.now() - prev
    if PRINT_TIME:
        logger.debug("Time computing clustering mmd: %s", elapsed)
    return mmd_dist
    ...
    for fake_g in fake_graphs:
        ...
        for fake_old in fake_evaluated:
            try:
                ...
                logger.warning("Timeout: Skipping this iteration")
                continue
            finally:
                # Disable the alarm
                signal.alarm(0)
...
class SpectreSamplingMetrics(nn.Module):
    ...
    def forward(
        ...
        if local_rank == 0:
            logger.info(
                "Computing sampling metrics between %d generated graphs and %d"
                " test graphs -- emd computation: %s",
                len(generated_graphs), len(reference_graphs), self.compute_emd,
            )
        networkx_graphs = []
        adjacency_matrices = []
        if local_rank == 0:
        ...
        if "wavelet" in self.metrics_list:
            if local_rank == 0:
                logger.info("Computing wavelet stats...")
        ...
        if "spectre" in self.metrics_list:
            if local_rank == 0:
                logger.info("Computing spectre stats...")
        ...
        if "clustering" in self.metrics_list:
            if local_rank == 0:
                logger.info("Computing clustering stats...")
        ...
        if "motif" in self.metrics_list:
            if local_rank == 0:
                logger.info("Computing motif stats...")
            motif = motif_stats(
                reference_graphs,
                networkx_graphs,
                motif_type="4cycle",
        ...
        if "orbit" in self.metrics_list:
            if local_rank == 0:
                logger.info("Computing orbit stats...")
        ...
        if "planar" in self.metrics_list:
            if local_rank == 0:
                logger.info("Computing planar accuracy...")
        ...
        if "tree" in self.metrics_list:
            if local_rank == 0:
                logger.info("Computing tree accuracy...")
        ...
        if (
            ...
            if local_rank == 0:
                logger.info("Computing all fractions...")
            if "sbm" in self.metrics_list:
        ...
        if local_rank == 0:
            logger.info("Sampling statistics: %s", to_log)
        try:
            if swanlab.run:
                swanlab.log(to_log)
        except Exception:
