...
class SamplingMolecularMetrics(nn.Module):
    ...
    def forward(
        ...
        if test and local_rank == 0:
            ...
            logger.debug("Generated smiles: %s", all_smiles)
            _export_smiles_csv(all_smiles, "final_smiles.csv", log_prefix="[SamplingMetrics]")

        if self.dataset_info.compute_fcd and len(self.test_smiles if test else self.val_smiles) > 0:
            to_log["fcd"] = compute_fcd(
                val_smiles=self.test_smiles if test else self.val_smiles,
                generated_smiles=all_smiles,
            )
        else:
            logger.info("FCD computation is disabled or val_smiles is empty. Skipping.")
            to_log["fcd"] = -1

        logger.info("fcd: %s", to_log["fcd"])

        # Skip distribution metrics for MPO (empty validation set)
        if len(self.test_smiles if test else self.val_smiles) > 0:
            logger.info("Starting custom metrics")
            ...
            logger.info("Skipping custom distribution metrics (MPO/No-Dataset mode).")
        if local_rank == 0:
            ...
            textfile.close()
            logger.info("Stability metrics: %s -- %s", stability, rdkit_metrics[0])
...
def compute_fcd(val_smiles, generated_smiles):
    ...
    if fcd is None:
        logger.warning("FCD package not installed. Skipping FCD computation.")
        return -1

    logger.info("Starting FCD computation")
    start = time.time()
    ...
    with warnings.catch_warnings():
        ...
        try:
            ...
            logger.error("Error in FCD computation. Setting FCD to -1.")
            fcd_score = -1

    end = time.time()
    logger.info("FCD computation time: %s, FCD score is %s", end - start, fcd_score)
...
def _export_smiles_csv(smiles_list, csv_path, log_prefix=""):
    ...
    if pd is not None:
        try:
            ...
            logger.warning("%spandas export failed (%s); falling back to csv module.", prefix, exc)
