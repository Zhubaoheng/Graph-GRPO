...
class NoiseDistribution:
    ...
    def update_limit_dist(self, node_dist, edge_dist):
        """
        Dynamically update the limit distribution (p0).
        Args:
            node_dist: tensor of shape (x_num_classes,) or (1, x_num_classes)
            edge_dist: tensor of shape (e_num_classes,) or (1, e_num_classes)
        """
        # Ensure input dimensions match internal state
        if node_dist.numel() != self.x_num_classes:
            # Handle potential mismatch due to virtual classes if absorbing
            if self.transition == "absorbing" and node_dist.numel() == self.x_num_classes - self.x_added_classes:
                 # Pad with 0 (or small eps) for the absorbing state if not provided
                 # Usually the absorbing state probability comes from the model definition, but here we update the data marginals.
                 # If node_dist is just the atom types, we might want to keep the absorbing state prob fixed or 0?
                 # Assuming node_dist is a valid new distribution over real classes.
                 # We keep the absorbing state as is (usually 0 for data distribution).
                 pad = torch.zeros(self.x_added_classes, device=node_dist.device, dtype=node_dist.dtype)
                 node_dist = torch.cat([node_dist, pad], dim=-1)
            else:
                 logger.warning("[NoiseDistribution] Node dist mismatch: got %d, expected %d", node_dist.numel(), self.x_num_classes)
                 return

        if edge_dist.numel() != self.e_num_classes:
             if self.transition == "absorbing" and edge_dist.numel() == self.e_num_classes - self.e_added_classes:
                 pad = torch.zeros(self.e_added_classes, device=edge_dist.device, dtype=edge_dist.dtype)
                 edge_dist = torch.cat([edge_dist, pad], dim=-1)
             else:
                 logger.warning("[NoiseDistribution] Edge dist mismatch: got %d, expected %d", edge_dist.numel(), self.e_num_classes)
                 return

        # Normalize
        node_dist = node_dist / (node_dist.sum() + 1e-9)
        edge_dist = edge_dist / (edge_dist.sum() + 1e-9)

        # Update
        self.limit_dist.X = node_dist.cpu() 
        self.limit_dist.E = edge_dist.cpu()
        
        # NOTE: self.limit_dist.y is typically uniform or fixed, we skip updating it for now unless needed.
