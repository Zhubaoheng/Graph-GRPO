...
class ExtraFeatures:
    ...
    def __call__(self, noisy_data):
        ...
        if self.features_type == "cycles":
            ...
            # Keep RRWP node features only for real nodes; zero out padding node features
            rrwp_node_attr = rrwp_node_attr * noisy_data["node_mask"].unsqueeze(-1)
            ...
            diag_index = torch.arange(rrwp_edge_attr.shape[1])
            rrwp_node_attr = rrwp_edge_attr[:, diag_index, diag_index, :]
            # Keep RRWP node features only for real nodes
            rrwp_node_attr = rrwp_node_attr * noisy_data["node_mask"].unsqueeze(-1)
            ...
            # Keep RRWP node features only for real nodes
            rrwp_node_attr = rrwp_node_attr * noisy_data["node_mask"].unsqueeze(-1)
            comp_rrwp_node_attr = comp_rrwp_node_attr * noisy_data["node_mask"].unsqueeze(-1)
