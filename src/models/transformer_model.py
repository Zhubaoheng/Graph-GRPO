...
class GraphTransformer(nn.Module):
    ...
    def forward(self, X, E, y, node_mask):
        ...
        # Gradient checkpointing: only used during training when gradients are enabled.
        # In eval mode, run directly to ensure consistency between sampling and recomputation.
        if self.training and torch.is_grad_enabled():
            # Training mode: use gradient checkpointing to save GPU memory
            def checkpoint_layer(layer_fn, x_input, e_input, y_input, mask_input):
                """Activation checkpoint wrapper for a single transformer layer."""
            ...
            for layer in self.tf_layers:
                X, E, y = checkpoint(
                    ...
                    use_reentrant=False  # Use non-reentrant version for better stability
                )
        else:
            # Inference mode: run directly to ensure numerical consistency
            for layer in self.tf_layers:
